...
- ✅ **Data quality and validation** — early checks in extract.py, validation in validate.py
- ✅ **Maintainability** — config-driven file paths and thresholds via config.yaml
- ✅ **Analytical readiness** — star schema warehouse design; Parquet outputs in the warehouse folder
...
```bash
foil_case_study/
├── run.py                           # Orchestrates the full ETL pipeline with logging
├── docs/                            # Documentation folder (data dictionary, architecture diagram)
│   ├── architecture_diagram.png       # ETL pipeline architecture diagram
│   ├── data_dictionary.md             # Data dictionary describing fact & dimension tables
│   ├── demo_pipeline.log              # Example log from a pipeline run
│   ├── data_profiling_report.csv      # Profiling report generated by extract.py
│
...
│   ├── extract.py                   # Data extraction from UCI repository (produces CSV + profiling report)
...
├── src/
│   └── helpers/
│       └── logger.py                # Centralized logging utility (not pushed to GitHub)
│
...
├── .gitignore                       # Excludes unnecessary files from version control (logs/, etc.)
